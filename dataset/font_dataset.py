...
class FontDataset(Dataset):
    ...
    def get_path(self):
        ...
        target_image_dir = "data_examples/train/TargetImage"  # 替换为实际的目录路径
    ...
    def __getitem__(self, index):
        ...
        parts = target_image_name.split('+')
        if len(parts) != 2:
            raise ValueError(f"Invalid target_image_name format: {target_image_name}")
        style, content = target_image_name.split('.')[0].split('+')
        
        # Read content image
        # Ensure root is correctly set

        content_image_path = f"data_examples/train/ContentImage/{content}.png"
